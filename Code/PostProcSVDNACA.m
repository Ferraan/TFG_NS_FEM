...
disp("********************************************")
disp("********************************************")
files=dir("GIDPOST/NACA0012_AoA_5NSS*");
...
for i=1:size(combinations,1)
    ...
    if(size(U1,2)>=size(U2,2)) %p>=q
        ...
        disp(cosangles);
        if(rows<length(cosangles)+2)
            rows=length(cosangles)+2;
        end
        %%%FIND Re and steps
        %Re
        pat="_U_";
...
for k=1:ntables
    ...
    begincol=columns+1;
    columns=columns+floor(size(angles,2)/ntables);
    disp("NEXT****************")
end
%%