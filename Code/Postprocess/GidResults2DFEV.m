function GidResults2DFE(NameFile,COOR,CONNECT,TypeElement,u,v,NAME_INPUT_DATA)
% INPUT DATA
%dbstop('6')
if nargin == 0
    load('tmp1.mat')
...
[~,posgp,~,~]=ComputeElementShapeFun(TypeElement,NNode,'K') ; 
if isempty(posgp)
    npg = 4 ;
else
    npg = size(posgp,2);
...
fprintf(fid_res,'Nodes not included\n');
%Se utiliza Given en lugar de Internal porque es mas general para distintos puntos de gauss que se
...
for istepLOC = 1:length(stepsSH)
    ...
    fprintf(fid_res, ['Result "Velocity"  "Load Analysis" '  num2str(time_step) ' Vector OnNodes  \n' ],[]);
    fprintf(fid_res,'ComponentNames "X-Velocity" "Y-Velocity"\n');
    fprintf(fid_res,'Values\n');
    fORMAT = ['%d',repmat(' %f %f',1,1),'\n'];
    fprintf(fid_res,fORMAT,[1:nnod;u;v]); 
