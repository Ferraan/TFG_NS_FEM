function GidResults2DFE(NameFile,COOR,CONNECT,TypeElement,p,NAME_INPUT_DATA)
% INPUT DATA
%dbstop('6')
if nargin == 0
    load('tmp1.mat')
...
for istepLOC = 1:length(stepsSH)
    ...
    fprintf(fid_res, ['Result "Pressure"  "Load Analysis" '  num2str(time_step) ' Scalar OnNodes  \n' ],[]);
    fprintf(fid_res,'ComponentNames "Pressure"\n');
    fprintf(fid_res,'Values\n');
    fORMAT = ['%d',repmat(' %f',1,1),'\n'];
    fprintf(fid_res,fORMAT,[1:nnod;p']); 
